...
public class Lab7 extends JPanel implements GLEventListener {
	 ...
	 * @throws IOException 
	 */
	public Texture textureItself;
	private Texture textureFromResource(String resourceName) throws IOException {
		URL textureSource = this.getClass().getResource(resourceName);
		BufferedImage image = ImageIO.read(Objects.requireNonNull(textureSource));
		GLContext glContext = displayGL.getContext();
		GL2 gl = glContext.getGL().getGL2();
		if (!glContext.isCurrent()) {
			glContext.makeCurrent();
		}
		textureItself = AWTTextureIO.newTexture(displayGL.getGLProfile(), image, true);
		textureItself.setTexParameteri(gl, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		textureItself.setTexParameteri(gl, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
		return textureItself;
	}
	...
	private Texture textureFromPainting() {
		...
		textureItself = AWTTextureIO.newTexture(displayGL.getGLProfile(), img, true);
		textureItself.setTexParameteri(gl, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		textureItself.setTexParameteri(gl, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
		return textureItself;
	}
	...
	public void display(GLAutoDrawable drawable) {    
		...
		textureItself = currentTexture;

		if (textureItself != null) {
			textureItself.enable(gl2);
			textureItself.bind(gl2);
			drawCurrentShape(gl2);
			textureItself.disable(gl2);
		} else {
			drawCurrentShape(gl2);
		}
